...
# Loop through each row of the ai.all.cnjtypes.p.values.my object
for (i in 1:nrow(ai.all.cnjtypes.p.values.my)) {
  # Find the index of the column with the highest value in this row
  max_col_index <- which.max(ai.all.cnjtypes.p.values.my[i,])
  # Store the column name in the ai.all.cnjtypes.predicted.my vector
...
# Loop through each row of the ti.all.cnjtypes.p.values.my object
for (i in 1:nrow(ti.all.cnjtypes.p.values.my)) {
  # Find the index of the column with the highest value in this row
  max_col_index <- which.max(ti.all.cnjtypes.p.values.my[i,])
  # Store the column name in the ti.all.cnjtypes.predicted.my vector
...
# Loop through each row of the ta.all.cnjtypes.p.values.my object
for (i in 1:nrow(ta.all.cnjtypes.p.values.my)) {
  # Find the index of the column with the highest value in this row
  max_col_index <- which.max(ta.all.cnjtypes.p.values.my[i,])
  # Store the column name in the ta.all.cnjtypes.predicted.my vector
