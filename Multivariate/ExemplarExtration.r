...
fviz_silhouette(hcut(ex.ti.ive.logic, k = 41, method = "ward.D2", hc_metric = "binary" ))
...
p.ti.AWive$cluster<-hcut(ex.ti.ive.logic, k = 42, method = "ward.D2" )$cluster
...
write.csv(ai.awive.exemplar[with(ai.awive.exemplar, order(cluster,p.val)),][myvars],"./ai.awive.ex.csv")
write.csv(ti.awive.exemplar[with(ti.awive.exemplar, order(cluster,p.val)),][myvars],"./ti.awive.ex.csv")
write.csv(ta.awive.exemplar[with(ta.awive.exemplar, order(cluster,p.val)),][myvars],"./ta.awive.ex.csv")
